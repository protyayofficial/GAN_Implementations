...
def parse_args():
    ...
    parser.add_argument('--c_lr', type=float, default=5e-5, help="Learning rate for the Critic.")
    ...
    parser.add_argument('--weight_clip', type=float, default=0.01, help="To enforce Lipschitz constraint.")
...
def save_image_grid(epoch, generator, device, num_classes, ncol=6):
    ...
    plt.xticks([])
    plt.yticks([])
    plt.savefig(f'results/WGAN_{epoch:03d}.jpg')
    plt.close()
...
def create_gif_and_move(output_dir):
    ...
    for filename in sorted(os.listdir('results')):
        ...
        if 'WGAN' in filename and filename.endswith('.jpg') or filename.endswith('.png'):
            img = Image.open(os.path.join('results', filename))
            images.append(img)
    
    # Create the GIF and save it in the output directory
    gif_path = os.path.join(output_dir, 'WGAN_MNIST.gif')
    if images:
...
def main():
    ...
    # Optimizers
    g_optim = RMSprop(generator.parameters(), lr=args.g_lr)
    c_optim = RMSprop(critic.parameters(), lr=args.c_lr)
